...
public class FlowControlSample1Solution {
    private static final Logger LOG = LoggerFactory.getLogger(FlowControlSample1Solution.class);

    public static void main(String[] args) {
        Flowable<Long> flowable = Flowable.interval(10, TimeUnit.MILLISECONDS);

        // the flowable above emits a sequential number every 10 milliseconds,
        // use a periodic sampling operator (after timestamp operator) to emit the most recent item every 1 second
        long startTime = System.currentTimeMillis();
        Flowable<Timed<Long>> result = flowable.timestamp()
