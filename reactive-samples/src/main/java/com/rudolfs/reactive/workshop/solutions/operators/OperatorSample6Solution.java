...
public class OperatorSample6Solution {
    private static final Logger LOG = LoggerFactory.getLogger(OperatorSample6Solution.class);

    public static void main(String[] args) {
        Flowable<Long> flowable = Flowable.just(5L, 2L);

        // use 1 operator to transform the flowable stream into stream1 that:
        ...
        Flowable<Long> stream1 = flowable.concatMap(aLong -> delayedLong(aLong));

        // use 1 operator to transform the flowable stream into stream2 that:
        ...
        Flowable<Long> stream2 = flowable.flatMap(aLong -> delayedLong(aLong));
